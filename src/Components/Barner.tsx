...
const Banner = async () => {
  const banners: BarnerData[] = await getBarnerData();
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-10 md:max-h-[600px]">
      ...
      <div className="flex flex-col space-y-5 md:space-y-10 h-auto md:max-h-[600px]">
        {banners.slice(1, 3).map((item: BarnerData) => (
          <div
            key={item._id}
          ...
          >
            <div className="w-1/2 flex flex-col">
              <div>
                ...
                <h2 className="text-3xl font-bold">{item?.subtitile}</h2>
              </div>
